...
The embedded version is **Lua 5.4.8**, with modifications. For security reasons it lacks concurrency, as well as module loading,
file access, pipes, command execution. The `coroutine`, `io` and `os` modules and their functions aren't available (but the
